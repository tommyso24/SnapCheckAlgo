...
// P4: Decide whether a regex-derived URL is trustworthy enough to commit
// as `extracted.companyUrl`. The regex can always find SOMETHING in a
// long body; "first URL scanned" on its own is too weak a signal because
// inquiry bodies routinely contain third-party links, product pages,
// article references, etc. We only accept the regex fallback when
// independent evidence ties the URL to the sender:
//
//   (a) URL's registered domain matches the sender email's registered
//       domain (ignoring free-email providers, which carry no signal);
//   (b) the URL appears in the last ~400 chars of the text alongside
//       classic signature markers (email, phone, "Tel:", "+<cc>-…",
//       WhatsApp, 微信) — i.e. it looks like the sender's sig block,
//       not a drive-by link buried in body prose.
//
// Otherwise leave companyUrl null. A missing companyUrl triggers
// `skipped` on downstream website/wayback panels — preferable to
// pointing the whole fanout at a wrong site.
export function isConfidentRegexFallback({ url, email, text }) {
  if (!url) return { accept: false, reason: 'no url' }
  const urlDomain = normalizeBareDomain(url)
  if (!urlDomain) return { accept: false, reason: 'url has no domain' }
  const urlLast2 = urlDomain.split('.').slice(-2).join('.')

  // Signal A: email-domain corroboration
  if (email && typeof email === 'string') {
    const em = email.toLowerCase().trim().match(/@([a-z0-9][a-z0-9.-]*\.[a-z]{2,})$/i)
    const emailDomain = em?.[1]
    if (emailDomain && !FREE_EMAIL_DOMAINS.has(emailDomain)) {
      const emailLast2 = emailDomain.split('.').slice(-2).join('.')
      if (urlLast2 === emailLast2) {
        return { accept: true, reason: 'url matches email domain' }
      }
    }
  }

  // Signal B: signature-block proximity
  if (text && typeof text === 'string') {
    const tail = text.slice(-400).toLowerCase()
    // URL appears in the tail (match on bare registered domain for
    // robustness — handles www., subdomain, trailing slash variants).
    const urlRe = new RegExp(urlLast2.replace(/\./g, '\\.'), 'i')
    if (urlRe.test(tail)) {
      // Sig markers: an @ (email), a "tel/phone/mob/whatsapp/微信" token,
      // or a +CC-style phone. Any single marker is enough alongside the
      // URL match — signature blocks are short and dense.
      const sigMarkers = /@|\btel\b|\bphone\b|\bmob\b|\bwhatsapp\b|微信|\+\d{1,3}[\s\-.()]*\d{3,}/i
      if (sigMarkers.test(tail)) {
        return { accept: true, reason: 'url in signature block' }
      }
    }
  }

  return { accept: false, reason: 'no corroborating signal' }
}
...
export async function extractEntities({ inquiry, websiteText, images, userUrl, ownDomains, baseUrl, apiKey, model, systemPrompt }) {
  ...
  //      (excluding any of the user's own domains), then confidence-gate
  //      the match against email-domain / signature-block evidence (P4).
  ...
  if (!extracted.companyUrl) {
    ...
    if (fromText) {
      const decision = isConfidentRegexFallback({
        url: fromText,
        email: extracted.email,
        text: combinedText,
      })
      if (decision.accept) {
        ...
        log.fallback({ source: 'regex_text', companyUrl: fromText, reason: decision.reason })
      } else {
        log.info('fallback_rejected', {
          source: 'regex_text',
          candidate: fromText,
