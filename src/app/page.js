...
function LoginPage({ onLogin }) {
  ...
  const labelStyle = {
    ...
    fontWeight: 600, letterSpacing: '0.09em', textTransform: 'uppercase', marginBottom: 8,
  }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: 'rgba(13,20,38,0.92)', border: '1px solid rgba(255,255,255,0.07)',
        ...
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          <div style={{
            ...
            background: 'linear-gradient(135deg,#1a56db,#0ea5e9)', marginBottom: 18,
          }}>
        ...
        {error && (
          <div style={{
            background: 'rgba(220,50,50,0.1)', border: '1px solid rgba(220,50,50,0.28)',
        ...
        <button onClick={handleLogin} disabled={loading} style={{
          ...
          background: loading ? 'rgba(26,86,219,0.45)' : 'linear-gradient(135deg,#1a56db,#0ea5e9)',
        ...
        <div style={{ textAlign: 'center', marginTop: 22, color: 'rgba(255,255,255,0.18)', fontSize: 11 }}>
          © 2025 mmldigi.com · 外贸背景调查系统
        </div>
      </div>
    </div>
  )
...
function QueryPage({ user }) {
  ...
  const extractRisk = t =>
    /高风险/.test(t) ? 'high' : /中风险/.test(t) ? 'medium' : /低风险/.test(t) ? 'low' : 'unknown'
  ...
  const sectionLabel = {
    ...
    letterSpacing: '0.11em', textTransform: 'uppercase', marginBottom: 20,
  }
  const inputBase = {
    width: '100%', background: 'rgba(255,255,255,0.04)',
    border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10,
    padding: '11px 14px', color: '#fff', fontSize: 14, lineHeight: 1.6,
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 26 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
        ...
        <div style={panelStyle}>
          <div style={sectionLabel}>输入信息</div>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <textarea value={inquiry} onChange={e => setInquiry(e.target.value)}
              placeholder="请粘贴完整的询盘邮件内容、买家信息等..." rows={11}
              style={inputBase} />
          </div>

          {error && (
            <div style={{
              background: 'rgba(220,50,50,0.09)', border: '1px solid rgba(220,50,50,0.25)',
              borderRadius: 8, padding: '9px 13px', marginBottom: 14, color: '#fc8181', fontSize: 13,
            }}>{error}</div>
          )}
        ...
        <div style={{ ...panelStyle, display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
            <div style={sectionLabel}>分析结果</div>
            {result && !streaming && (
              <span style={{
                fontSize: 10.5, fontWeight: 700, padding: '3px 10px', borderRadius: 20,
                background: `${riskColor[extractRisk(result)]}18`,
                color: riskColor[extractRisk(result)],
                border: `1px solid ${riskColor[extractRisk(result)]}38`,
                letterSpacing: '0.04em',
              }}>{riskLabel[extractRisk(result)]}</span>
            )}
            {streaming && (
          ...
          <div ref={resultRef} style={{
            flex: 1, minHeight: 380, background: 'rgba(0,0,0,0.18)', borderRadius: 10,
            padding: 16, overflowY: 'auto', whiteSpace: 'pre-wrap',
            color: result ? 'rgba(255,255,255,0.82)' : 'rgba(255,255,255,0.2)',
...
function HistoryPage({ user }) {
  ...
  const visible = queries.filter(q =>
    !search || q.url?.includes(search) || q.inquiry?.includes(search) || q.userEmail?.includes(search)
  )
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: 60, color: 'rgba(255,255,255,0.3)' }}><Spinner size={24} /></div>
      ) : visible.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {visible.map((q, i) => (
            <div key={i} onClick={() => setSelected(selected === i ? null : i)} style={{
              background: selected === i ? 'rgba(26,86,219,0.09)' : 'rgba(255,255,255,0.028)',
              border: `1px solid ${selected === i ? 'rgba(26,86,219,0.28)' : 'rgba(255,255,255,0.065)'}`,
              borderRadius: 12, padding: '15px 18px', cursor: 'pointer', transition: 'all 0.18s',
            }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{
                  fontSize: 10.5, fontWeight: 700, padding: '2px 9px', borderRadius: 20, flexShrink: 0,
                  background: `${riskColor[q.riskLevel]}15`, color: riskColor[q.riskLevel],
                  border: `1px solid ${riskColor[q.riskLevel]}33`,
                }}>{riskLabel[q.riskLevel]}</span>
              ...
              {selected === i && (
                <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {q.url && <div>
                    <div style={{ color: 'rgba(255,255,255,0.28)', fontSize: 10.5, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>公司网址</div>
                    <div style={{ color: '#60a5fa', fontSize: 12.5, fontFamily: 'monospace' }}>{q.url}</div>
                  </div>}
                  {q.inquiry && <div>
                    <div style={{ color: 'rgba(255,255,255,0.28)', fontSize: 10.5, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>询盘信息</div>
                    <div style={{ color: 'rgba(255,255,255,0.55)', fontSize: 12.5, lineHeight: 1.65, background: 'rgba(0,0,0,0.18)', borderRadius: 8, padding: '9px 12px', maxHeight: 110, overflowY: 'auto', whiteSpace: 'pre-wrap' }}>{q.inquiry}</div>
                  </div>}
                  <div>
                    <div style={{ color: 'rgba(255,255,255,0.28)', fontSize: 10.5, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>分析结果</div>
                    <div style={{ color: 'rgba(255,255,255,0.78)', fontSize: 12.5, lineHeight: 1.8, background: 'rgba(0,0,0,0.18)', borderRadius: 8, padding: '12px 14px', maxHeight: 280, overflowY: 'auto', whiteSpace: 'pre-wrap' }}>{q.result}</div>
                  </div>
                </div>
              )}
            </div>
...
// ─── ADMIN PAGE ────────────────────────────────────────────────────────────────
function AdminPage() {
  const [form, setForm] = useState({ baseUrl: '', apiKey: '', modelName: '', systemPrompt: '' })
  const [loading, setLoading] = useState(true)
  ...
  const label = { display: 'block', color: 'rgba(255,255,255,0.4)', fontSize: 10.5, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8 }
  const input = { width: '100%', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: '11px 14px', color: '#fff', fontSize: 13, marginBottom: 22 }

  if (loading) return <div style={{ textAlign: 'center', padding: 80 }}><Spinner size={28} /></div>

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 26 }}>
      <div>
        <h1 style={{ color: '#fff', fontSize: 25, fontWeight: 700, letterSpacing: '-0.5px' }}>系统设置</h1>
        <p style={{ color: 'rgba(255,255,255,0.38)', fontSize: 13.5, marginTop: 5 }}>配置 AI 分析引擎参数</p>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.6fr', gap: 18 }}>
        <div style={panel}>
          <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 10.5, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 24 }}>API 配置</div>

          <label style={label}>Base URL</label>
          <input value={form.baseUrl} onChange={e => setForm({ ...form, baseUrl: e.target.value })}
            placeholder="https://ai.example.com/v1" style={{ ...input, fontFamily: "'DM Mono',monospace", fontSize: 12 }} />

          <label style={label}>API Key</label>
          <div style={{ position: 'relative', marginBottom: 22 }}>
            <input type={showKey ? 'text' : 'password'} value={form.apiKey} onChange={e => setForm({ ...form, apiKey: e.target.value })}
              placeholder="sk-..." style={{ ...input, marginBottom: 0, paddingRight: 44, fontFamily: "'DM Mono',monospace", fontSize: 12 }} />
          ...
          <input value={form.modelName} onChange={e => setForm({ ...form, modelName: e.target.value })}
            placeholder="gemini-2.0-flash" style={{ ...input, fontFamily: "'DM Mono',monospace", fontSize: 12 }} />

          <div style={{ background: 'rgba(26,86,219,0.08)', border: '1px solid rgba(26,86,219,0.2)', borderRadius: 8, padding: '10px 13px' }}>
            <div style={{ color: '#60a5fa', fontSize: 11.5, lineHeight: 1.6 }}>
              支持任何 OpenAI 兼容接口，包括 Gemini、DeepSeek、通义千问等。Base URL 填写到 <code style={{ fontFamily: 'monospace', fontSize: 11 }}>/v1</code> 即可。
            </div>
          </div>
        </div>

        <div style={panel}>
          <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 10.5, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8 }}>Secret Prompt</div>
          <div style={{ color: 'rgba(255,255,255,0.28)', fontSize: 12, marginBottom: 16, lineHeight: 1.6 }}>
            此提示词对用户完全隐藏，决定 AI 如何分析外贸背调信息。
          </div>
          <textarea value={form.systemPrompt} onChange={e => setForm({ ...form, systemPrompt: e.target.value })}
            rows={17} style={{ ...input, marginBottom: 0, lineHeight: 1.75, fontSize: 12.5 }} />
        </div>
      </div>

      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        <button onClick={save} disabled={saving} style={{
          padding: '11px 30px', border: 'none', borderRadius: 10, cursor: saving ? 'wait' : 'pointer',
        ...
        {saved && (
          <div style={{ color: '#68d391', fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      <aside style={{
        ...
        background: 'rgba(255,255,255,0.018)', borderRight: '1px solid rgba(255,255,255,0.055)',
        display: 'flex', flexDirection: 'column', padding: '26px 14px',
      }}>
        ...
        <nav style={{ flex: 1 }}>
          {nav.map(item => (
            <button key={item.id} onClick={() => setPage(item.id)} style={{
              ...
              background: page === item.id ? 'rgba(26,86,219,0.18)' : 'transparent',
              border: `1px solid ${page === item.id ? 'rgba(26,86,219,0.28)' : 'transparent'}`,
              color: page === item.id ? '#60a5fa' : 'rgba(255,255,255,0.45)',
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.055)', paddingTop: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '0 4px', marginBottom: 10 }}>
            <div style={{ width: 30, height: 30, borderRadius: 7, background: 'rgba(26,86,219,0.28)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#60a5fa', fontSize: 11.5, fontWeight: 700, flexShrink: 0 }}>
              {user.name?.[0] || 'U'}
            </div>
            <div style={{ minWidth: 0 }}>
              ...
              <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: 10.5 }}>{user.role === 'admin' ? '管理员' : '用户'}</div>
            </div>
          </div>
          <button onClick={onLogout} style={{
            width: '100%', padding: '7px 11px', background: 'transparent',
            border: '1px solid rgba(255,255,255,0.075)', borderRadius: 7,
            color: 'rgba(255,255,255,0.38)', fontSize: 12.5, cursor: 'pointer',
...
export default function App() {
  ...
  return (
    <Layout user={user} onLogout={logout} page={page} setPage={setPage}>
      {content}
    </Layout>
  )
}
